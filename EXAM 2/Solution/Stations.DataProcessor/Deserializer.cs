...
namespace Stations.DataProcessor
{
	public static class Deserializer
	{
		...
		public static string ImportStations(StationsDbContext context, string jsonString)
		{
			...
			foreach (var x in deserializedStations)
			{
				if (!IsValid(x))
				{
				...
				var stationAlreadyExists = validStations.Any(s => s.Name == x.Name);
				...
				var station = Mapper.Map<Station>(x);

				validStations.Add(station);

				sb.AppendLine(string.Format(SuccessMessage, x.Name));
			}
		...
		public static string ImportClasses(StationsDbContext context, string jsonString)
		{
			...
			foreach (var x in deserializedClasses)
			{
				if (!IsValid(x)) 
				{
					sb.AppendLine(FailureMessage);
				...
				var seatingClassAlreadyExists = validSeatingClasses
					.Any(sc => sc.Name == x.Name || sc.Abbreviation == x.Abbreviation);
				...
				var seatingClass = Mapper.Map<SeatingClass>(x);
		...
		public static string ImportTrains(StationsDbContext context, string jsonString)
		{
			...
			foreach (var x in deserializedTrains)
			{
				if (!IsValid(x))
				{
					sb.AppendLine(FailureMessage);
				...
				var trainAlreadyExists = validTrains.Any(t => t.TrainNumber == x.TrainNumber);
				...
				var seatsAreValid = x.Seats.All(IsValid); 
				if (!seatsAreValid)
				{
					sb.AppendLine(FailureMessage);
					continue;
				...
				var seatingClassesAreValid = x.Seats 
				...
				var type = Enum.Parse<TrainType>(x.Type);

				var trainSeats = x.Seats.Select(s => new TrainSeat 
					{
						SeatingClass =
				...
				sb.AppendLine(string.Format(SuccessMessage, x.TrainNumber));
			}
		...
		public static string ImportCards(StationsDbContext context, string xmlString)
		{
            ...
            foreach (var x in deserializedCards)
            {
                if (!IsValid(x))
                {
                    sb.AppendLine(FailureMessage);
                ...
                var cardType = Enum.TryParse<CardType>(x.CardType, out var card) ? card : CardType.Normal;
